...
PERCENT_TO_NEGATE = 0.02
PERCENT_TO_ZERO = 0.01
...
def select_parent(population):
    ...
    # total_fitness = sum(network.fitness for network in population)
    # Calculate the relative fitness of each individual
    # relative_fitness = [f.get_fitness_value() / total_fitness for f in population]
    ...
    # chosen_index = random.choices(range(len(relative_fitness)), weights=relative_fitness, k=1)[0]
...
def uniform_crossover(parent1, parent2):
    child = NeuralNetwork()
    #todo - cancel automatic initialization in NN class
    child.weights = []
    length = len(parent1.weights)
    for i in range(length):
        # Randomly choose whether to inherit the weight from parent 1 or parent 2
...
def mutate(network):
    ...
    for i in range(length):
        ...
        # if random.uniform(0, 1) < PERCENT_TO_NEGATE:
        #     network.weights[i] = network.weights[i] * -1
        # if random.uniform(0, 1) < PERCENT_TO_ZERO:
        #     network.weights[i] = 0

    # for _ in range(NUM_OF_MUTATIONS):
    #     # Mutate weights within the input-hidden layer
    #     mutate_layer(network, "input_hidden")
    # Mutate weights within the hidden-output layer
    # mutate_layer(network, "hidden_output")


# def mutate_layer(network, layer):
#     # Get the indices of weights corresponding to the specified layer
#     if layer == "input_hidden":
#         start = 0
#         end = NUM_HIDDEN * (NUM_INPUTS + 1)
#     elif layer == "hidden_output":
#         start = NUM_HIDDEN * (NUM_INPUTS + 1)
#         end = start + NUM_HIDDEN + 1
#     else:
#         raise ValueError("Invalid layer specified")

# weights = network.weights[start:end]
# # Choose two random weights to switch
# index1, index2 = random.sample(range(len(weights)), 2)
# # Perform the switch
# #todo network.weights?
...
def train_test_split(data_x, data_y, test_percent=0.2):
    # Combine data_x and data_y into a list of tuples
    combined_data = list(zip(data_x, data_y))
    # Shuffle the combined data
    random.shuffle(combined_data)
    # Calculate the index to split the data
    split_index = int(len(combined_data) * (1 - test_percent))
...
# todo - change the input to get two files - train file and test file, as files and not as data
def buildnet0(train_x, train_y, test_x, test_y):
...
# todo - change the input to get two files - train file and test file, as files and not as data
def buildnet1(train_x, train_y, test_x, test_y):
    global MUTATION_RATE
    global NUM_OF_MUTATIONS
    # Initialize the population with random neural networks
    population = [NeuralNetwork() for _ in range(POPULATION_SIZE)]
    ...
    # Save the best-performing network
    best_network = population[0]
    with open('wnet1.txt', 'w') as file:  # Change file name accordingly
        for weight in best_network.weights:
            file.write(str(weight) + '\n')
...
def runnet0(weights_file, unclassified_data_file):
    ...
    # todo - check it contains all the data correctly
    data_x, data_y = load_data(unclassified_data_file)  # Change file name accordingly
    test_data = list(zip(data_x, data_y))
    ...
    with open('testnet0_predictions.txt', 'w') as file:
        for prediction in test_predictions:
            ...
            i += 1
    # todo - erase
    print("prediction rate of new data:" + str(counter / len(test_y) * 100) + "%")
...
def check_prediction(weights_file, test_x, test_y):
    ...
    for prediction in test_predictions:
        if prediction == test_y[i]:
            counter += 1
        i += 1
    print("prediction rate:" + str(counter / len(test_y) * 100) + "%")


if __name__ == '__main__':
    data_x, data_y = load_data('nn0.txt')  # Change file name accordingly
    # Split the shuffled data into training and testing sets
    train_x, train_y, test_x, test_y = train_test_split(data_x, data_y, 0.2)
    # Genetic Algorithm
    # buildnet0(train_x, train_y, test_x, test_y)
    # check_prediction('wnet0.txt', test_x, test_y)
    # todo - examine it on a file named testnet0:
    # runnet0('wnet0.txt', 'testnet0')
    buildnet1(train_x, train_y, test_x, test_y)
    check_prediction('wnet1.txt', test_x, test_y)
    # runnet1('wnet1.txt', 'testnet1')
